...
export const syncHistory = async (
  ...
  try {
    ...
    if (data.mediaType === "movie") {
      media = await tmdb.movies.details(mediaId);
    } else if (data.mediaType === "tv") {
      media = await tmdb.tvShows.details(mediaId);
    } else {
      const { fetchAnimeById } = await import("./anime");
      media = await fetchAnimeById(mediaId);
    }
      ...
      .upsert(
        {
          ...
          adult:
            data.mediaType === "anime"
              ? media.nsfw === "black" || media.nsfw === "gray"
              : "adult" in media
                ? media.adult
                : false,
          backdrop_path: data.mediaType === "anime" ? media.main_picture?.large : media.backdrop_path,
          poster_path:
            data.mediaType === "anime"
              ? media.main_picture?.large || media.main_picture?.medium
              : media.poster_path,
          release_date:
            data.mediaType === "anime"
              ? media.start_date
          ...
          vote_average: data.mediaType === "anime" ? media.mean || 0 : media.vote_average,
        },
        {

...
import { DISCOVER_ANIME_VALID_QUERY_TYPES } from "@/types/anime";
import { parseAsSet } from "@/utils/parsers";
import { useQueryState, parseAsStringLiteral } from "nuqs";
import { useCallback, useMemo, useEffect } from "react";

const VALID_CONTENT_TYPES = ["multi", "movie", "tv", "anime"] as const;
...
const useDiscoverFilters = () => {
  ...
  const [content, setContent] = useQueryState(
    "content",
    parseAsStringLiteral(VALID_CONTENT_TYPES).withDefault("multi"),
  );

  const types = useMemo(
    () => [
      { name: "Discover", key: DEFAULT_QUERY_TYPE },
      ...(content === "multi" ? [] : content === "movie" ? movies : content === "tv" ? tvShows : anime).map(({ name, param }) => ({
        name: name.replace(/(Movies|TV Shows|Anime)/g, "").trim(),
        key: param,
      })),
    ],
